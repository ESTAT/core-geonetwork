...
public class MetadataNotifierManager {
    ...
    class UpdateTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
                    Log.debug(Geonet.DATA_MANAGER, "Notifier - updateMetadata before (uuid): " + uuid);
                }
                ...
                for (MetadataNotifier service : loadNotifiers()) {
                    ...
                    try {
                        ...
                        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
                            Log.debug(Geonet.DATA_MANAGER, "Notifier - updateMetadata (uuid): " + uuid);
                        }
                        ...
                        Log.error(Geonet.DATA_MANAGER, "Notifier - updateMetadata ERROR (uuid): " + uuid + "notifier url "
                ...
                Log.error(Geonet.DATA_MANAGER, "Notifier error: " + e.getMessage(), e);
            }
        }
    }
        ...
        public void run() {
            try {
                ...
                for (MetadataNotifier service : loadNotifiers()) {
                    ...
                    try {
                        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
                            Log.debug(Geonet.DATA_MANAGER, "Notifier - deleteMetadata before (uuid): " + uuid);
                        }
                        client.webDelete(service, uuid);
                        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
                            Log.debug(Geonet.DATA_MANAGER, "Notifier - deleteMetadata (uuid): " + uuid);
                        }

                        // mark metadata as notified for current notifier service
                        setMetadataNotified(metadataId, service, true);
                    } catch (Exception ex) {
                        Log.error(Geonet.DATA_MANAGER, "Notifier - deleteMetadata ERROR (uuid): " + uuid + " " + ex.getMessage(), ex);
    ...
    private Map<Metadata, MetadataNotification> getUnnotifiedMetadata(int notifierId,
        ...
        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
            Log.debug(Geonet.DATA_MANAGER, "Notifier - getUnnotifiedMetadata start");
        }
        final ConfigurableApplicationContext applicationContext = ApplicationContextHolder.get();
        ...
        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
            Log.debug(Geonet.DATA_MANAGER, "Notifier - getUnnotifiedMetadata returning #" + notificationMap.size() + " results");
        }
    ...
    private void setMetadataNotified(int metadataId, MetadataNotifier notifier,
        ...
        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
            Log.debug(Geonet.DATA_MANAGER, "Notifier - setMetadataNotified finished for metadata with id " + metadataId + "and notifier with id "
                                           + notifier.getId());
        }
    }
    ...
    private void setMetadataNotifiedError(final int metadataId, final MetadataNotifier notifier,
        ...
        if (Log.isDebugEnabled(Geonet.DATA_MANAGER)) {
            Log.debug(Geonet.DATA_MANAGER, "Notifier - setMetadataNotifiedError");
        }
        try {
            final ConfigurableApplicationContext applicationContext = ApplicationContextHolder.get();
            ...
            if (Log.isDebugEnabled(Geonet.DATA_MANAGER))
                Log.debug(Geonet.DATA_MANAGER, "Notifier - setMetadataNotifiedError finished for metadata with id " + metadataId + "and notitifer " +
